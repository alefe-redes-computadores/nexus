...
export default function Header() {
  ...
  async function handleLogout() {
    try {
      ...
      // Destruição total de qualquer cache local
      localStorage.clear();
      ...
      // Recarrega a página violentamente
      window.location.reload();
    }
  }

  // Múltiplas tentativas de achar a foto
  ...
  return (
    <header className="flex flex-col relative px-6 pt-6 pb-4 bg-zinc-950/90 backdrop-blur-xl border-b border-zinc-900 sticky top-0 z-30">
      
      {/* MODO DETETIVE: Só para descobrirmos o bug da foto! Depois vamos tirar isso. */}
      {user && (
        <div className="absolute top-0 left-0 w-full bg-red-900/80 text-[10px] text-white p-1 break-all text-center z-50 font-mono">
          URL: {avatar ? avatar : 'NENHUMA URL ENCONTRADA'} | Erro: {imgError ? 'SIM' : 'NÃO'}
        </div>
      )}
